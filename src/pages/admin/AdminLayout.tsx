...
import { LogOut, LayoutDashboard, FileText, Home, Music, MailOpen, Gift, ClipboardList, Users, ShoppingBag, Globe, Menu } from 'lucide-react';
...
const AdminLayout: React.FC = () => {
  ...
  return (
    <div data-no-reveal className="flex min-h-screen bg-brand-light dark:bg-brand-dark text-brand-dark dark:text-brand-light">
      <aside className="hidden md:flex w-64 shrink-0 bg-brand-light dark:bg-brand-dark-alt border-r border-brand-secondary/50 p-6 flex-col shadow-lg">
        {sidebar}
      </aside>

      <Sheet open={menuOpen} onOpenChange={setMenuOpen}>
        <SheetContent side="left" className="w-72 p-6 flex flex-col bg-brand-light dark:bg-brand-dark-alt overflow-y-auto">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="bg-brand-light dark:bg-brand-dark border-b border-brand-secondary/50 p-4 shadow-sm flex items-center justify-between">
          <Button variant="ghost" size="icon" className="md:hidden mr-2" onClick={() => setMenuOpen(true)} aria-label="Open admin menu">
            <Menu className="h-5 w-5" />
          </Button>
          <h1 className="flex-1 text-xl md:text-2xl font-bold text-brand-dark dark:text-brand-light">
            {getHeaderTitle(location.pathname)}
          </h1>
          <div className="flex items-center gap-4">
            <span className="hidden sm:inline text-brand-dark/80 dark:text-brand-light/80 text-sm">
