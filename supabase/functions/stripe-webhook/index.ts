  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (type === 'arrangement_purchase') {
        console.log(`[stripe-webhook] Processing arrangement purchase for session: ${session.id}`);
        const arrangementIds = session.metadata?.arrangement_ids?.split(',') || [];
        const buyerEmail = session.customer_details?.email;

        if (!buyerEmail) {
          console.error(`[stripe-webhook] No buyer email found for session ${session.id}. Skipping arrangement email.`);
          return new Response(JSON.stringify({ received: true, message: 'No buyer email, skipping arrangement email.' }), {
            status: 200,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }

        // Trigger email with download links
        const { error: emailError } = await supabaseClient.functions.invoke('send-arrangement-email', {
          body: {
        ...
        if (emailError) {
          console.error(`[stripe-webhook] Error invoking send-arrangement-email: ${emailError.message}`);
        } else {
          console.log(`[stripe-webhook] Triggered arrangement email for session ${session.id}.`);
        }
      } else {
        // Default to gift card logic if no type or type is gift card
        console.log(`[stripe-webhook] Processing gift card purchase for session: ${session.id}`);
